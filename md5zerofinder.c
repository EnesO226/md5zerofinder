...
#define THREAD_COUNT 16  // Number of threads to create

// Function prototypes for functions like md5initK, md5init, md5round, md5str, etc.
// Assume these are defined elsewhere as required by run_md5_simd_demo

typedef struct {
    char input[16];
    int thread_id;  // Unique ID for the thread
    long long start_index; // Starting index for the thread
} ThreadData;

// Declare MD5 SIMD demo function
void run_md5_simd_demo(char *input, long long individualIndex);

// Thread function that will call the MD5 SIMD demo function
void* thread_md5_simd_demo(void* arg) {
    ThreadData* data = (ThreadData*)arg;
    //printf("Thread %d starting at index %lld\n", data->thread_id, data->start_index);

    // Run the MD5 SIMD demo with the input string and start index
    run_md5_simd_demo(data->input, data->start_index);  // Pass the start index to the demo function
...
void run_md5_simd_demo(char *input, long long individualIndex) {
    ...
    // Copy up to 15 characters of the input string to userstring
    long argv1len = strlen(input);
    if (argv1len > 15) argv1len = 15;
    memcpy(userstring, input, argv1len);

    // Pad the remaining characters with '0'
    for (i = argv1len; i < 15; i++) {
        userstring[i] = '0';
    }
    clock_t start_time = clock();
    // Set up message blocks based on the input userstring
    ...
    unsigned long long j, iterations = 0xfffffffffffffff;
    unsigned int msg6, msg7, msg4, msg5;
    char *md5str_ptr;
    long long IntPointer;
    FILE *hashes_file = NULL;  // Declare the file pointer here

    // Open the file for appending before the loop
    hashes_file = fopen("hashes.txt", "a");
    if (hashes_file == NULL) {
        perror("Error opening file");
        exit(1);
    }
    for (unsigned long long i = individualIndex; i < iterations; i+=1) {
        long long temp = i;
        msg6 = 0x41414141 + (temp & 0xf) + (((temp & 0xf0) << 4)) + (((temp & 0xf00) << 8)) + (((temp & 0xf000) << 12));
        msg7 = 0x42434445 + ((temp & 0xf0000) >> 16) + (((temp & 0xf00000) >> 12)) + (((temp & 0xf000000) >> 8)) + (((temp & 0xf0000000) >> 4));
        ...
        msg4 = 0x41414141 + ((temp + 2) & 0xf) + ((((temp + 2) & 0xf0) << 4)) + ((((temp + 2) & 0xf00) << 8)) + ((((temp + 2) & 0xf000) << 12));
        msg5 = 0x46474849 + (((temp + 3) & 0xf0000) >> 16) + ((((temp + 3) & 0xf00000) >> 12)) + ((((temp + 3) & 0xf000000) >> 8)) + ((((temp + 3) & 0xf0000000) >> 4));

        msg[4] = X_mm256_setr_epi32(msg4, msg4, msg4, msg4, msg4, msg4, msg4, msg4);
        msg[5] = X_mm256_setr_epi32(msg5, msg5, msg5, msg5, msg5, msg5, msg5, msg5);
        ...
        for (j = 0; j < 8; j++) {
            if (((res_union.intelement[j]) == 0x00000000)) {
              
                fprintf(hashes_file, "%s%llu%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n", 
                userstring, j + 1,
                msg4 & 0xff, (msg4 >> 8) & 0xff, (msg4 >> 16) & 0xff, (msg4 >> 24),
                msg5 & 0xff, (msg5 >> 8) & 0xff, (msg5 >> 16) & 0xff, (msg5 >> 24),
                msg6 & 0xff, (msg6 >> 8) & 0xff, (msg6 >> 16) & 0xff, (msg6 >> 24),
                msg7 & 0xff, (msg7 >> 8) & 0xff, (msg7 >> 16) & 0xff, (msg7 >> 24));
              
                printf("\n\n%s%llu%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n", 
                userstring, j + 1,
                msg4 & 0xff, (msg4 >> 8) & 0xff, (msg4 >> 16) & 0xff, (msg4 >> 24),
                msg5 & 0xff, (msg5 >> 8) & 0xff, (msg5 >> 16) & 0xff, (msg5 >> 24),
                msg6 & 0xff, (msg6 >> 8) & 0xff, (msg6 >> 16) & 0xff, (msg6 >> 24),
                ...
                double hash_rate = (double)(double)((IntPointer) / elapsed_seconds / 1000000);

                md5str_ptr = md5str(Endian32(res_union.intelement[j]), Endian32(myvars.b.intelement[j]),
                                    Endian32(myvars.c.intelement[j]), Endian32(myvars.d.intelement[j]));
                fprintf(hashes_file, "%s\n", md5str_ptr);

                printf("%s\n", md5str_ptr);
                printf("%llu @ %.3f Mh/s\n\n", IntPointer, hash_rate);
    ...
    //printf("md5 Hash Rate approx. = %f MH/s\n", 8.0f * iterations / (1000000 * duration));
    //free(msg);
}

int main(int argc, char *argv[]) {
    if (argc <= 1) {
        printf("Usage: %s somestring\nThis program demonstrates the capability of SIMD with multithreading.\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
        // Prepare a unique input for each thread by modifying argv[1] slightly
        snprintf(thread_data[i].input, 16, "%s_%d", argv[1], i*256);  // Add thread ID suffix
        thread_data[i].thread_id = i;

        // Calculate a different starting index for each thread
        thread_data[i].start_index = i * 0x0000000000000001; // Just an example, adjust as needed

        if (pthread_create(&threads[i], NULL, thread_md5_simd_demo, &thread_data[i]) != 0) {
